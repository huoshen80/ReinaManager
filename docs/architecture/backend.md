...
少量启动设置使用 `tauri-plugin-store` 的 `settings.json`。封面、存档备份、数据库备份和安装中间文件存于文件系统。数据库备份目录在运行时解析失败时记录警告并回退默认目录，同时保留用户配置；存档备份根目录变更时，目标根目录可访问则静默迁移，目标根目录不存在、暂时不可访问或配置无法在当前机器解析则保存原始配置、跳过迁移并记录警告。备份文件删除失败或文件不存在时保留数据库记录。
...
存档备份根目录变更必须调用 `change_savedata_backup_root`。该 command 与创建、恢复、删除和历史清理共用存档操作锁；目标根目录可访问时按“复制到临时目录、逐文件校验、切换临时目录、更新配置、删除旧目录”的顺序静默迁移。目标根目录不存在、暂时不可访问、旧根目录不可访问或路径暂时无法解析时，仅更新原始配置、跳过迁移并记录警告，不创建目标根目录、不删除旧目录、不清理数据库记录；前端提示用户旧备份位置。普通 `update_settings` 拒绝直接修改 `save_root_path`。迁移前会清理明确返回 `NotFound` 的单个失效备份记录；权限或网络错误不会触发清理。迁移失败时配置保持不变，配置已切换但旧目录清理失败时返回残留目录警告。新旧目录经词法规范化和符号链接解析后不得相同之外的父子重叠。
