...
pub(crate) async fn finalize_monitored_session<R: Runtime>(
    ...
    let duration = calculate_session_duration(
    ...
    match duration {
        Ok(Some(duration)) => {
            ...
            let session_data = (
                ...
                i32::try_from(duration.duration_minutes),
            );

            match session_data {
                (Ok(game_id), Ok(start_time), Ok(end_time), Ok(duration)) => {
                    ...
                    .await
                    {
                        Ok(session) => {
                            ...
                            info!(
                                ...
                                game_id, session.session_id, duration
                            );
                        }
                        Err(error) => {
