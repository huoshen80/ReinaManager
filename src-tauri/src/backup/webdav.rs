...
/// 清理远程自动备份（保留最新的 max_count 份）
pub async fn cleanup_auto_backups(
    url: &str,
    username: &str,
    password: &str,
    root: &Option<String>,
    ...
    let backups = list_backups(url, username, password, root).await?;

    // 只筛选自动备份
    let mut auto_backups: Vec<_> = backups
        .into_iter()
        .filter(|b| b.name.contains("_auto_"))
        .collect();
    auto_backups.sort_by(|a, b| a.name.cmp(&b.name));

    if auto_backups.len() <= max_count {
        return Ok(Vec::new());
    }

    let to_delete = auto_backups.len() - max_count;
    let mut deleted = Vec::new();
    for backup in auto_backups.into_iter().take(to_delete) {
        delete_backup(url, username, password, root, &backup.name).await?;
